...
// reworked to having dropdown typing
// temp addition. improved to have scrolling, can only spawn DOWN, and allows for typing
...
class FlxUIDropDownMenuEx extends FlxUIGroup implements IFlxUIWidget implements IFlxUIClickable implements IHasParams
{
	...
	var dropdownFilter:Null<String> = null;
			...
			?UIControlCallback:Bool->FlxUIDropDownMenuEx->Void)
	{
		...
		if (!header.text.onTextChange.has(onHeaderTextChange)) header.text.onTextChange.add(onHeaderTextChange);
	...
	function onHeaderTextChange(v:String, change:FlxInputTextChange)
	{
		//
		if (change != DELETE_ACTION)
		{
			dropdownFilter = v;
			updateButtonPositions();
		}
	...
	private function updateButtonPositions():Void
	{
		...
		var filteredList = list.copy();
		if (dropdownFilter != null)
		{
			filteredList = filteredList.filter((str->str.label.text.startsWith(dropdownFilter)));
		}
		
		currentScroll = FlxMath.maxInt(0, FlxMath.minInt(filteredList.length - 1, currentScroll));
		...
		{
			var button:FlxUIButton = list[i];
			if (button != null && filteredList.contains(button) && i >= currentScroll)
			{
				button.y = offset;
				offset += buttonHeight;
				visCount++;
		...
		dropPanel.resize(header.background.width, dropdownFilter != null ? visCount * buttonHeight : getPanelHeight());
	}
	...
	private function showList(b:Bool):Void
	{
		...
		dropdownFilter = null;
		if (!b) header.text.endFocus();
		else if (!header.text.hasFocus) header.text.startFocus();
...
class FlxUIDropDownHeader extends FlxUIGroup
{
	 ...
	 */
	public function new(Width:Int = 120, ?Background:FlxSprite, ?Text:FlxInputText, ?Button:FlxUISpriteButton)
	{
		super();
		...
		if (text == null)
		{
			text = new FlxInputText(0, 0, Std.int(background.width - button.frameWidth));
		}
